...
;(function() {
	...
	window.onload = function(){
		...
		// bodywrap.parentNode.insertBefore(msc,bodywrap)

		// 修改snippet footer但展示
		// console.log($(".cacher-snippet-footer"))
		// console.log($("#microw > div:nth-child(5) > div"))
		let res = getSnippet();
		let resArray = []
		let i = 0
		res.then(response=>{
		...
		document.querySelectorAll("#microw .cacher-snippet").forEach((element)=>{
			element.setAttribute("class","is-issue-content");
			
			let foot = element.lastChild;
			// element.lastChild.innerHTML="🍓 Yummy🥝";
			let author = document.createElement("em");
			author.innerText="🍓 Yummy🥝 ";
			element.insertBefore(author,element.firstChild);

			foot.innerHTML= "";
			let postTime = document.createElement("em");
			postTime.innerText= resArray[i]["createdAt"].replace("T", " ").replace("Z", " ");
			// foot.appendChild(author);
			foot.appendChild(postTime);
			// let lastconten = element.querySelector(".ca-hljs table > tbody > tr:last-child td:last-child");
			// element.querySelector(".ca-hljs table > tbody > tr:last-child").remove();
			// element.lastChild.appendChild(lastconten);
			// console.log(resArray[i]);
			foot.setAttribute("id", "snippetFoot");
			// element.lastChild.innerHTML="🍓 Yummy🥝 "+resArray[i]["createdAt"].replace("T", " ").replace("Z", " ");
			i ++;
		});
	})
	}
	...
	async function getSnippet(){
		let res = await axios.get("https://cors-anywhere.herokuapp.com/https://api.cacher.io/libraries/cb7723cbce059cfa0f7d/snippets/7d52434c5531f01447c0", {
			"headers": {
